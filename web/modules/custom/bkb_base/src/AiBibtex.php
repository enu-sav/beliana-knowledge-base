...
final class AiBibtex {
   ...
   */
  public function getBibtexPerplexity($source, $prompt): string {
    // Get the Perplexity provider
    $provider = $this->aiProvider->createInstance('perplexity');
    $token = $source->get('label')->value;
    $promptReplaced = str_replace('[source:title]', $token, $prompt);
    ...
    // Get the response
    $output = $provider->chat($input, $this->configFactory->get('ai_perplexity.settings')
      ->get('default_model'));
   ...
   *
   * @param \Drupal\Core\Entity\EntityInterface $source
   *   The source entity whose label will be appended to the prompt.
   * @param string $prompt
   ...
   */
  public function getBibtexOepnAI($source, $prompt): string {
    ...
    $content = $guzzle->request('POST', 'https://api.openai.com/v1/chat/completions', [
      ...
      'json' => [
        ...
        'messages' => [
          [
            'role' => 'user',
            'content' => $prompt . ' ' . $source->get('label')->value,
          ],
        ],
      ],
    ]);
