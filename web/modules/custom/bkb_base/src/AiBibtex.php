...
final class AiBibtex {
   ...
   */
  public function getBibtex($ai, $label, $prompt, $model = ''): string {
    switch ($ai) {
      case 'perplexity':
        return $this->getBibtexPerplexity($label, $prompt, $model);
      case 'open_ai':
        return $this->getBibtexOpenAI($label, $prompt, $model);
    }
   ...
   *   The BibTeX record generated by the AI provider.
   */
  private function getBibtexPerplexity($label, $prompt, $model): string {
   ...
   */
  private function getBibtexOpenAI($label, $prompt, $model): string {
    ...
    if ($response) {
      ...
      $response_text = isset($response['choices']) ? $response['choices'][0]['message']['content'] : '';
